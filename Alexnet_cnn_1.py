...
def Imp_DS():
    ...
    y_train = encode[0:int(0.8*len(set_yy))]
    
    x_test = set_xx[int(0.8*len(set_xx)):len(set_xx)]
    y_test = encode[int(0.8*len(set_yy)):len(set_yy)]
...
def Conv_Net(x, W, B):
    ...
    n1 = tf.layers.batch_normalization(pool1)
     #normalize?
         #centering?
    #convolution layer 2
    conv2 = tf.nn.conv2d(n1, W['W2'], strides = (1,1,1,1), padding = 'SAME')
    ...
    n2 = tf.layers.batch_normalization(relu2)
         #normalize?
    #convolution layer 3
    conv3 = tf.nn.conv2d(n2, W['W3'], strides = (1,1,1,1), padding = 'SAME')
    conv3 = tf.nn.bias_add(conv3, B['B3'])
    relu3 = tf.nn.relu(conv3)
    n3 = tf.layers.batch_normalization(relu3)
    
    #convolution layer 4
    conv4 = tf.nn.conv2d(n3, W['W4'], strides = (1,1,1,1), padding = 'SAME')
    conv4 = tf.nn.bias_add(conv4, B['B4'])
    relu4 = tf.nn.relu(conv4)
    n4 = tf.layers.batch_normalization(relu4)
    
    #convolution layer 5
    conv5 = tf.nn.conv2d(n4, W['W5'], strides = (1,1,1,1), padding = 'SAME')
    conv5 = tf.nn.bias_add(conv5, B['B5'])
    relu5 = tf.nn.relu(conv5) 
    pool5 = tf.nn.max_pool(relu5, ksize = (1,3,3,1), strides = (1,2,2,1), padding = 'VALID')
    n5 = tf.layers.batch_normalization(pool5)
    
    #Reshape input to interact with FC layer weights
    fc_in = tf.contrib.layers.flatten(n5)
    ...
    drop1 = tf.nn.dropout(relu6, rate = 0.5)   #Use drop-out only when training in progress
...
x_train, y_train, x_test, y_test = Imp_DS()
